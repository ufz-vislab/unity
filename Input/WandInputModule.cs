	...
	public class WandInputModule : BaseInputModule
	{
		...
		public void MyProcess()
		{
			...
			if (!ignoreInputsWhenLookAway || ignoreInputsWhenLookAway && currentOverGo != null)
			{
				...
				else if (IOC.Core.Instance.Input.IsOkButtonPressed())
				{
					ClearSelection();
					eventData.pressPosition = eventData.position;
					eventData.pointerPressRaycast = eventData.pointerCurrentRaycast;
			...
			var newVal = IOC.Core.Instance.Input.GetHorizontalAxis();
			...
			{
				_controlAxisUsed = true;
				var time = IOC.Core.Instance.Time.Time(); // Time.unscaledTime
